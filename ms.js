...
class Game {
  ...
  getNeighbors(i) {
    ...
    for (let y1 = Math.max(y - 1, 0); y1 < Math.min(y + 2, this.height); y1++) {
      for (
...
class Solver {
  ...
  bruteSolve(linAlg = true) {
    ...
    const binomInactive = binomFactory(closedCells.length - activeCells.length);
      ...
      while (stack.length > 0) {
        ...
        if (newMines > this.game.mines) {
          ...
          for (let y = 0; y < a1.length; y++) {
            ...
            if (rhs > max || rhs < 0 || (stack.length >= n && rhs !== 0)) {
              valid = false;
            }
          }
        }
    ...
    const setProbability = (i, probability) => {
    ...
    const generateCombinations = (curr, i) => {
      if (i === groupCounts.length) {
        const sum = curr.reduce((acc, [size]) => acc + size, 0);

        if (
          this.game.mines - sum > closedCells.length - activeCells.length ||
          sum > this.game.mines
        ) {
          return;
        }

        const prod = BigInt(curr.reduce((acc, [, count]) => acc * count, 1));
        const toAdd = (binomInactive(this.game.mines - sum) || 1n) * prod;
        totalCount += toAdd;

        curr.forEach(([size, count], idx) => {
          for (const [cell, counts] of Object.entries(groupCellCounts[idx])) {
            if (!(size in counts)) {
              continue;
            }
            if (cell in cellCounts) {
              ...
              cellCounts[cell] = (toAdd * BigInt(counts[size])) / BigInt(count);
            }
          }
        });
      ...
      for (const [size, count] of Object.entries(groupCounts[i])) {
        generateCombinations([...curr, [parseInt(size), count]], i + 1);
      }
    };
    ...
    if (closedCells.length - activeCells.length !== 0) {
      const inactiveProb = bigIntDivide(
        Object.entries(counts).reduce(
          (acc, [size, count]) => acc + count * BigInt(startMines - size),
          0n
        ),
  ...
  guess(probabilities = undefined) {
    ...
    for (let i in probabilities) {
      ...
      if (
        ...
        (isBestActive === isActive && numActiveNeighbors > maxActiveNeighbors)
      ) {
        best = i;
  ...
  update() {
    ...
    for (let x = 0; x < this.game.width; x++) {
      for (let y = 0; y < this.game.height; y++) {
        this.game.set(this.game.xytoi(x, y), this.getValue(this.getCell(x, y)));
      }
    }
...
class MinesweeperOnline extends Website {
  ...
  inject() {
    ...
    this.prefix =
      document.getElementsByClassName("fa-moon-o").length === 6 ? "hdd" : "hd";
    document.head.insertAdjacentHTML(
      "beforeend",
      "<style>" +
    ...
    return [100, 10, 1].reduce(
      (acc, elem) =>
        ...
        parseInt(
          elem *
            Array.from(
              document.getElementById("top_area_mines_" + elem).classList
            )
              .find((name) => name.startsWith(this.prefix + "_top-area-num"))
              .slice(-1)
        ),
      0
    );
  ...
  getState() {
    if (
      ...
      document.getElementsByClassName(this.prefix + "_top-area-face-win").length
    ) {
      return WON;
    }
    return IN_PROGRESS;
...
class MinesweeperOne extends Website {
  constructor() {
    ...
    this.timeDigits[0] =
      qp1 +
      qp5 +
      "OYSPKct8ApqDEcpqHNL6rfxtYQdSZGVSimkEgOtGCPzWT1vHaQKWa6ryoX5CD5CIuYxQD" +
      "VlJEuQlCgA7";
    this.timeDigits[1] =
      qp1 +
      qp5 +
      "N4SPCbHN95g0ApoJRHVJ79ZpF3h4VrhhpTgqbMRlyKSCNHWG47N+fL9D5YAxog30Cj4+n" +
      "JpCUQAAOw==";
    this.timeDigits[2] =
      qp1 +
      qp5 +
      "PISPKct8AZoTLzZrIEQuJ0sBmrJUZDhypfetkFSGIHxQMLWtyQbuotFB8IKenM7UE/4wk" +
      "mVk5hjeZJ9PAQA7";
    this.timeDigits[3] =
      qp1 +
      qp5 +
      "OYSPKct8AZoTLzZrIEQuJ0sBmrJUZDhypfetkFSGIFzBlLyRXm50Yuv6qXBAlPDge12Ct" +
      "gjSJvt8CgA7";
    this.timeDigits[4] =
      qp1 +
      qp5 +
      "PISPCbHNJwybJgJh6bXYQlyBSBd65TmKZMqZX7V1oCfXB1PPt7JJDbtLrDQfzwPoEIp+G" +
      "tGD8jBBfzxeAQA7";
    this.timeDigits[5] =
      qp1 +
      qp5 +
      "OYSPKct8ApoLZsUGaLWIapdoF9RlG1meoXklhghJFixzZh0dIsJ5ir1TmYIgFq9nFN4wS" +
      "ByqZnO5CgA7";
    this.timeDigits[6] =
      qp1 +
      qp5 +
      "OYSPKct8ApoLZsUGaLWIapdoF9RlG1meoXklhghJFixz0YiRNsKBT78bqXhA1NB3jOV0O" +
      "AeqFnQhCgA7";
    this.timeDigits[7] =
      qp1 +
      qp5 +
      "OYSPKct8AZoTLzZrIEQuJ0sBmrJUZDhypfet27iFHoy84h2Z5HzqR5f6rXAqFNHQwaVcv" +
      "FjtFvxICwA7";
    this.timeDigits[8] =
      qp1 +
      "DQAXAPAAAAAAAP8AACwAAAAADQAXAAACNISPGct8AZqDEcpqHNL6rfxtYQdSZGVSiok2k" +
      "SeFb2yBiY2uqVr24q7zwWyXUWsCi92WhgIAOw==";
    this.timeDigits[9] =
      qp1 +
      qp5 +
      "OoSPKct8ApqDEcpqHNL6rfxtYQdSZGVSiok2kSeFQNBOB51g8y6qeG+g/Uq3nEa48tV0t" +
      "JgFFstJDQUAOw==";
    this.timeNeg =
      qp1 +
      qp5 +
      "MoSPCbHN95g0rzkKba4VcY19ytdd2JgJ6ipo7Ju+rZiQtVae5pah4S2a4H4RzyWnQBQAA" +
      "Ds=";
    this.cellOpenIm = new Array(9);
    this.cellOpenIm[0] =
      qp1 +
      "EAAQAPAAAHt7e729vSwAAAAAEAAQAAACHYSPacHtD550cc5qYZZ4s+6Bm5iRlnl5lNqgH" +
      "MsUADs=";
    this.cellOpenIm[1] =
      qp1 +
    ...
    this.cellOpenIm[4] =
      ...
      qp4 +
      "LoyPacLtH550sYErLqBOZ9xU3zZyjFeGp8aCQtW26oQy4lO/U2rv+b2bBU3DRgEAOw==";
    this.cellOpenIm[5] =
      qp1 +
    ...
    this.cellOpenIm[8] =
      qp1 +
      "EAAQAPAAAHt7e729vSwAAAAAEAAQAAACKYSPacHtD550kanV6j3ZYl8F2id+4UZlV1qGb" +
      "feqJ7hacsJOkqvDvVMAADs=";
    this.bombFlagged =
      qp1 +
      "EAAPAPIAAAAAAHt7e729vf8AAP///wAAAAAAAAAAACwAAAAAEAAPAAADOUi63NJwiaCEv" +
      "dgGSvIY2VZZ3xdyZAliYpeaLIrBV5sJwDliebzjgKDPFSwWNTKjEfm7MSebqFSaAAA7";
    this.bombDisarmed =
      "data:image/gif;base64,R0lGODlhEAAQAPIEAAAAAHsAAHt7e729vf///wAAAAAAAAA" +
      "AACH5BAUKAAQALAAAAAAQABAAAAIrlI9pw+0vnnTxhTAruzc33knaAHoWJk6A+aypA8Qu" +
      "1cj2PGi3TE+s3wM6CgA7";
    this.blankCell =
      qp1 +
      qp3 +
    ...
    this.faceDead =
      qp1 +
      "GgAaAPIAAAAAAHt7e729vf//AP///wAAAAAAAAAAACwAAAAAGgAaAAADgBi63M4ikEmrv" +
      "TNKzK/eQiiOJMlkZVqehAq8b8qW72Db8TiPwO3jugUq1Pv9AKKdoNgrDprAD4h4c+KqoZ" +
      "0Tet1KUVbj8dtais9I7VmcFpbDsJqvrRhuj1iyiMmOKs1rT0luJGFzQXVlPHBIiFM0cTK" +
      "EKpRZQlIPmQ2Ymp2cnZoJADs=";
    this.faceSmile =
      qp1 +
      "GgAaAPIAAAAAAHt7e729vf//AP///wAAAAAAAAAAACwAAAAAGgAaAAADfxi63M4ikEmrv" +
      "TNKzK/eQiiOJMlkZVqehAq8b8qW72Db8TiPwO3jugUq1Pv9AKKdoGg8hpTMGi76FLaWPu" +
      "lAu/2AsM0mcscMZ72oshlIvqlxbvQVvD3G5aIyjIvE59ddVQpDRGZ9gl88ajlJVi57Mo4" +
      "qk4gtD5eYXpibAZqcmAkAOw==";
    this.faceWin =
      qp1 +
      "GgAaAPIAAAAAAHt7AHt7e729vf//AP///wAAAAAAACwAAAAAGgAaAAADhCi63M5DlEmrv" +
      "TNKzK/eQyiOJMlkZVqehQq8b8qWL2Hb8TiPwO3jugUq1Pv9AKLdoGg8hpRFmLRGQH5AS5" +
      "y0uq1eUb0wjFsLf1vLYoDKDWjPxKYcuWPKfXQhOos73qxQeF1AcCJ2TVaFhneJioaHOUl" +
      "6LlIykyqYT0JXD50NnJ6hoKGeCQA7";
    this.facePirate =
      qp1 +
      "GgAaAPIAAAAAAHt7e729vf//AP///4CAAAAAAAAAACwAAAAAGgAaAAADhxi63M4ikEmrv" +
      "TNKzK/eQiiOJMlkZVqehAq8b8qW7zAAdjzOI27fv9tugQr5coWgMMQT+AqvpHLZ9MGsRw" +
      "CT2HL+rrmjkGeFAqRTLTk6TX9A3nZbzTXK5+9iVpndKvRBV2JaeV12d0uFJGJ8Q3+GIjW" +
      "BhCJNi1cydSqbfhJvD6ANn6Gko6ShCQA7";
  }
  ...
  getMineCount() {
    ...
    if (hundreds === this.timeNeg) {
      return 10 * this.timeDigits.indexOf(tens) + this.timeDigits.indexOf(ones);
    }
    return (
